...
def inference(
    ...
    grid_z:list[torch.Tensor] = torch.randn([n_shape, G.z_dim], device=device).split(1)  # random code for geometry
    ...
    print(f'Random code for geometry, size of list: {len(grid_z)}')
    print(f'Random code for texture, size of list: {len(grid_tex_z)}')
    ...
    if inference_save_interpolation:
        ...
        save_visualization_for_interpolation(G_ema,num_sam=30, save_dir=os.path.join(run_dir, 'interpolation'))
